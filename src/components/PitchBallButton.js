...
const PitchBallButton = ({ results, setResults, pitchSpeed, spinRate, xpos, ypos, plateX, plateZ, pfxX, pfxZ, zone, pitchType, popupVisible, setPopupVisible }) => {
  ...
  const sendPredictionRequest = async () => {
    ...
    try {
      const logisticResponse = await fetch('http://localhost:5001/predict-logistic', {
        method: 'POST',
      ...
      // For Naive Bayes
      const naiveResponse = await fetch('http://localhost:5002/predict-naive-bayes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload)
      });
...
marginLeft: '47%'}}>
    ...
    {results.logistic && <div>Logistic Regression Result: {results.logistic}</div>}
    {results.naiveBayes && <div>Naive Bayes Result: {results.naiveBayes}</div>}
</div>
